...
for name, val in [("Depth","0.2"), ("Width","0.2"), ("Thickness","0.006"), ("Radius","0")]:
    p = ParameterString(); p.Name=name; p.Value=val; css_col.Parameters.Add(p)
...
for name, val in [("Width","0.5"), ("Height","0.5")]:
    p = ParameterString(); p.Name=name; p.Value=val; css_ped.Parameters.Add(p)
...
# BoltGrid with 8 anchors M20 (IsAnchor=True)
bolt_grid = BoltGrid()
...
m.AddObject(lg)
lc = LoadCase(); lc.Id=1; lc.Name="ULS"; lc.LoadType=LoadCaseType.Variable; lc.Type=LoadCaseSubType.VariableStatic
...
# Save IOM
iom_path = r"C:\Users\aintc\OneDrive\Escritorio\Tenorious\col109_complete_v3.xml"
m.SaveToXmlFile(iom_path)
print("IOM saved:", os.path.getsize(iom_path), "bytes")

# Verify
xml_content = open(iom_path, 'r', encoding='utf-16').read()
print("ConcreteBlockData with OutlinePoints:", "OutlinePoints" in xml_content)
print("BoltGrid with IsAnchor=True:", "IsAnchor>true" in xml_content)
...
res_path = r"C:\Users\aintc\OneDrive\Escritorio\Tenorious\col109_results_v3.xmlR"
...
out_path = r"C:\Users\aintc\OneDrive\Escritorio\Tenorious\col109_complete_v3.ideaCon"
